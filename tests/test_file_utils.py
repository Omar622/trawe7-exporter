...
def test_filter_audio_files():
    files = ['a.mp3', 'b.wav', 'c.txt', 'd.mp3', 'e.m4a']
    assert filter_audio_files(files) == ['a.mp3', 'b.wav', 'd.mp3', 'e.m4a']
...
def test_list_audio_files_in_dir():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert set(found) == set([files[0], files[1], files[3], files[4]])
